...
export function AdoptApplyUserTest() {
    ...
    const initialGenter = useMemo(() => {
      ...
      return genderArr.find(item => item.english === state.gender)?.name
    },[state?.gender])
    ...
    const handleUpdateUser = _.throttle(handleUpdate, 3000, { 'trailing': false });
    ...
    const checkUserSetting = useCallback((obj: Partial<ApplyUser>): obj is Required<ApplyUser> =>{
        const keys = ['name', 'age', 'gender', 'address', 'phoneNumber', 'job'];
        let isTrue = true;
        for (let x of keys) {
            if (!obj.hasOwnProperty(x)) isTrue = false;
        }
        return isTrue;
    },[])
    ...
    useEffect(() => {
        console.log('state',checkUserSetting(state))
        if (checkUserSetting(state)) {
            ...
            <form className='border  p-2 rounded-md'>
                <div className="grid md:grid-cols-3 md:gap-6 mt-4 md:justify-center md:items-center">
                    <div className="relative z-0 w-full mb-6 group">
                        <input type="text" name="name" id="name" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-memyo-yellow4 focus:outline-none focus:ring-0 focus:border-memyo-yellow4" placeholder=" " value={state?.name || ''} onChange={handleUpdateUser}/>
                    ...
                    <div className="relative z-0 w-full mb-6 group">
                        <input type="text" name="age" id="phoneNumber" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-memyo-yellow4" value={state?.age || ''} onChange={handleUpdateUser} placeholder='00'/>
                ...
                <div className="relative z-0 w-full mb-6 group">
                        <input type="tel" pattern="[0-9]{3}-[0-9]{3}-[0-9]{4}" name="phoneNumber" id="phoneNumber" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-memyo-yellow4" placeholder="010-0000-0000" value={state?.phoneNumber || ''} onChange={handleUpdateUser}/>
                ...
                <div className="w-full">
                    <div className="relative z-0 w-full mb-6 group">
                        <input type="text" name="job" id="job" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-memyo-yellow4" placeholder=" "  value={state?.job || ''} onChange={handleUpdateUser}/>
                ...
                <div className="w-full">
                    <div className="relative z-0 w-full mb-6 group">
                        <input type="text" name="address" id="address" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-memyo-yellow4" placeholder=" "  value={state?.address || ''} onChange={handleUpdateUser} />
