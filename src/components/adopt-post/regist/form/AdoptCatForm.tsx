...
function AdoptCatForm({ cat }: Props) {
  ...
  const neuteredArr = [
    { name: '아니오', english: 'NOT_NEUTERED' },
    { name: '모름', english: 'UNKNOWN' },
  ...
  const [neutered, setNeutered] = useState<string>(() => {
    if (catVal?.neutered)
      return neuteredArr.find((neuter) => neuter.english === catVal.neutered)!.name;
    else return '아니오';
  });
  ...
  useEffect(() => {
    ...
    if (state.neutered == null) {
      handleCatForm({ neutered: 'NOT_NEUTERED' });
    }
    if (state.sex == null) {
          ...
          <div className="flex flex-wrap -mx-3 mb-2 md:space-y-5">
            ...
            <div className="w-full px-3 mb-6 md:mb-0">
              ...
              <input
                ...
                placeholder=""
                value={state?.weight || ''}
                onChange={(e) => handleCatForm({ weight: parseInt(e.target.value) })}
              />
            </div>
            <div className="w-full px-3">
              <label
