...
function AdoptUploadCatPic() {
    ...
    return (
        <div className='w-full lg:h-screen h-auto md:mb-0 mb-20' onDrop={onFileDragUpdate} onDragEnter={onFileDrag} onDragOver={onFileDragOver} >
                <h3 className="text-center font-bold mb-2">유기묘 사진</h3>
                <label form="cat-image" className='md:w-3/5 w-full p-2 h-screen'>
                    <div className="flex flex-col items-center justify-center pt-5 pb-6 w-full h-full bg-gray-100 hover:bg-gray-200">
                        {catImage.length === 0 &&
                                (<>
                        ...
                        {catImage.length > 0 &&
                            (<div className='flex text-sm gap-3 flex-wrap' >
                                <div id="preview" className='w-full h-full grid grid-cols-3 gap-1'>
                                    {urlArr.length > 0 && urlArr.map((url, i) => (<CatImg url={Object.values(url)[0]} image={catImage[i]} key={i} id={Object.keys(url)[0]} handleDelete={removeFile} />))}
                                </div>    
                            </div>)
                        }    
                    </div>
