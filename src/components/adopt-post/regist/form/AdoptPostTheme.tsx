...
function AdoptPostTheme({ title, content }: Props) {
  ...
  const handleTheme = useCallback(
    <T extends HTMLInputElement | HTMLTextAreaElement>(e: React.ChangeEvent<T>) => {
  ...
  return (
    <>
      ...
      <div className="p-2 mt-5">
        ...
        <label htmlFor="title" className="block mt-2 py-2 text-gray-700 w-full mb-2">
          타이틀
        </label>
        <input
          id="title"
          name="title"
          className="border-b appearance-none bg-transparent w-full text-gray-700 mr-3 py-1 px-2 leading-tight focus:outline-none"
          type="text"
          defaultValue={registTitle}
          onChange={handleTheme}
        />
        <label htmlFor="content" className="block mt-2 py-2 text-gray-700">
          설명
        </label>
        <textarea
          id="content"
